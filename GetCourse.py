...
if __name__ == "__main__":
    ...
    # 查詢教師資料
    print("\n查詢課程資料...")
    course_response = soap_service.GetCourse(id_key)

    if course_response:
        # 轉換為字串，確保能夠儲存
        course_response_str = str(course_response)
        save_to_file("GetCourse.txt", course_response_str)
import re
...
# 執行檔案處理
process_file("GetCourse.txt", "GetCourse1.txt")
...
def convert_txt_to_csv(txt_filename, csv_filename):
    """讀取 GetCourse1.txt，解析多筆資料並轉換成 GetCourse.csv，移除特殊字元"""
    headers = ["CCODE","CNAME","TEACHER_CODE","YEAR_NO","SEM_NO","COURSE_ENAME","CREDIT","STAR_DATE","END_DATE","SYLLABUS_URL","NET_COURSE_WEEK","TEACH_HOUR","COURSE_KIND","COURSE_TYPE"]
...
# 執行轉換
convert_txt_to_csv("GetCourse1.txt", "GetCourse.csv")
file_path = "GetCourse1.txt" 
if os.path.exists(file_path):
    os.remove(file_path)
file_path = "GetCourse.txt" 
if os.path.exists(file_path):
    os.remove(file_path)
