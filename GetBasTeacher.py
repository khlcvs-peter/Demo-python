...
if __name__ == "__main__":
    ...
    if teacher_response:
        ...
        save_to_file("GetBasTeacher.txt", teacher_response_str)
...
# 執行檔案處理
process_file("GetBasTeacher.txt", "GetBasTeacher1.txt")
...
def convert_txt_to_csv(txt_filename, csv_filename):
    """讀取 GetBasTeacher1.txt，解析多筆資料並轉換成 GetBasTeacher1.csv"""
    headers = ["TEACHER_CODE", "NAME", "GENDER", "EMAIL",  "ACCOUNT", "IS_TEACHER", "EDU_DEPT"]
    data_list = []  # 存放多筆資料
    data_dict = {}  # 暫存單筆資料
...
# 執行轉換
convert_txt_to_csv("GetBasTeacher1.txt", "GetBasTeacher.csv")
file_path = "GetBasTeacher1.txt" 
if os.path.exists(file_path):
    os.remove(file_path)
file_path = "GetBasTeacher.txt" 
if os.path.exists(file_path):
    os.remove(file_path)
