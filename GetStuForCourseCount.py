...
def main():
    print("這是 查詢課程選課統計學生人數資料.py 的主要功能！")
...
class SoapWebService:
    ...
    def GetStuForCourseCount(self, id_key):
        """查詢課程基本資料"""
        return self.call_api("GetStuForCourseCount", IDKey=id_key)
...
if __name__ == "__main__":
    ...
    # 查詢教師資料
    print("\n查詢課程資料...")
    course_response = soap_service.GetStuForCourseCount(id_key)

    if course_response:
        # 轉換為字串，確保能夠儲存
        course_response_str = str(course_response)
        save_to_file("GetStuForCourseCount.txt", course_response_str)
import re
...
def clean_text(text):
    ...
    text = text.replace("GetCourseResult", "")
...
# 執行清理
clean_file("GetStuForCourseCount.txt")
...
# 執行檔案處理
process_file("GetStuForCourseCount.txt", "GetStuForCourseCount1.txt")
...
def convert_txt_to_csv(txt_filename, csv_filename):
    """讀取 GetStuForCourseCount1.txt，解析多筆資料並轉換成 GetStuForCourseCount1.csv，移除特殊字元"""
    headers = ["CCODE","CNT"]
    #headers = ["CCODE", "CNAME", "TEACHER_CODE", "YEAR_NO", "COURSE_ENAME","CREDIT", "TEACH_HOUR",,"COURSE_TYPE"]
    data_list = []  # 存放多筆資料
    data_dict = {}  # 暫存單筆資料
...
# 執行轉換
convert_txt_to_csv("GetStuForCourseCount1.txt", "GetStuForCourseCount1.csv")
file_path = "GetStuForCourseCount1.txt" 
if os.path.exists(file_path):
    os.remove(file_path)
file_path = "GetStuForCourseCount.txt" 
if os.path.exists(file_path):
    os.remove(file_path)
